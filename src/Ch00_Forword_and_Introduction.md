...
此版本的教材，假定安装了 Rust `1.67.1` （发布于 2023-02-09）或更新版本。请参阅 [第 1 章的 “安装” 小节](docs/Ch01_Getting_Started.md#安装) 进行安装，或对已安装的 Rust 进行升级。
...
通过消除原有各种陷阱，并提供友好、全新工具集一路帮助咱们，Rust 破除了编写这类苛刻程序中的诸多障碍。那些需要“深入”到底层控制的程序员们，现在就可以运用 Rust，在无需承担一直以来的崩溃或者安全漏洞的下，并无须深入细致地掌握变化无常工具链，就可以达成他们的目标了。更了不起的是，设计这门语言就是为了引导咱们编写出在运行速度及内存使用方面有效率的可靠代码。
