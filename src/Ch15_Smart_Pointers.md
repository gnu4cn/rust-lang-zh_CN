...
报错显示此类型“具有无限大小，has infinite size”。该错误指出这个类型 “有着无穷大小（has infinite size）。”原因在于这里已把 `List` 定义为了有着一个递归的变种：其直接拥有其自身类型的另一个值。如此一来，Rust 就无法找出他需要多少空间来存储一个 `List` 类型值。接下来就要深究收到这个错误的原因。首先，这里将审视一下，Rust 是怎样确定出，他需要多少内存空间来存储某个非递归类型值。
