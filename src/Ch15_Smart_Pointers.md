...
这个库将提供跟踪某个值接近最大值的程度，及在什么时刻发出什么消息的功能。使用这个库的应用，将被期望提供发送消息的机制：应用可以在应用中中放置消息、发送电子邮件、发出手机短信或其他东西。这个库不需要知道这个细节。他所需的只是实现咱们将提供的名为 `Messenger` 特质的东西。以下清单 15-20 给出库的代码：


文件名：`src/lib.rs`

```rust
{{#rustdoc_include projects/limit_tracker/src/lib.rs::38}}
```
