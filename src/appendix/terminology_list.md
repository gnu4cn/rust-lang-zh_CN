...
- 单态化
    ...
    所谓 *单态化，monomorphization*，是指即通过把在编译后用到的具体类型填入到泛型位置，而将通用代码转换为具体代码的过程。参考 [用到泛型代码的性能问题](../generic_types_traits_and_lifetimes/generics.md#用到泛型的代码的性能问题)。


- 内聚属性

    a property called *coherence*，参见 [对类型实现特质](../generic_types_traits_and_lifetimes/traits.md#对类型实现特质)。


- 孤儿规则

    the orphan rule, 参见 [对类型实现特质](../generic_types_traits_and_lifetimes/traits.md#对类型实现特质)。


- `impl Trait` 语法

    `impl Trait` syntax, 在函数参数清单中，将特质用作参数类型注解的语法。参见：[将特质用作参数](../generic_types_traits_and_lifetimes/traits.md#将特质用作参数)


- 特质边界语法

    Trait bound syntax, 参见 [特质边界语法](../generic_types_traits_and_lifetimes/traits.md#特质边界语法)


- 语法糖

    Sugar syntax, 参见 [特质边界语法](../generic_types_traits_and_lifetimes/traits.html#特质边界语法)


- 指明多个特质边界的 `+` 语法

    The `+` syntax for specifying multiple trait bounds, 参见：[`+` 语法下的多个特质边界](../generic_types_traits_and_lifetimes/traits.md#-语法下的多个特质边界)


- `where` 子句

    `where` clauses, 参见 [`where` 字句下更清楚的特质边界](../generic_types_traits_and_lifetimes/traits.md#where-子句下更清楚的特质边界)
