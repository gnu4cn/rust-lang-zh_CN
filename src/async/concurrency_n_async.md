...
除了发送消息，我们还需接收他们。在本例中，由于我们知道有多少条消息进来，我们可以通过调用 `rx.recv().await` 四次，手动完成接收。但在现实世界中，我们通常将等待 *未知* 数量的消息，因此我们需要一直等待，直到确定没有更多信息为止。


在清单 16-10 中，我们使用了一个 `for` 循环，处理从同步通道接收到的所有项目。然而，Rust 还没有一种为一系列异步项，编写 `for` 循环的方法，因此我们需要使用一种以前从未见过的循环：`while let` 的条件循环。这是我们在 [“使用 `if let` 与 `let else` 的简明控制流]() 小节中，循环版本。只要指定的模式继续匹配值，循环就会继续执行。
