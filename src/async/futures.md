...
Rust 中异步编程的关键要素，是 `futures` 以及 Rust 的 `async` 与 `await` 关键字。


所谓一个 `future`，是个现在可能还没有准备好，但会在未来某个时刻准备好的值。(在许多语言中，都有这同样的概念，有时会以其他名称如 `task` 或 `promise` 出现）。Rust 提供了`Future` 特质，作为一个构建块，这样不同异步操作，就可以不同数据结构，但却以通用接口实现。在 Rust 中，`futures` 是一些实现了 `Future` 特质的类型。每个 `future` 都保存着其自己已完成进度的信息，以及何谓 “就绪” 的含义。

咱们可将 `async` 关键字，应用于代码块及函数，指定出他们可被中断与恢复。在某个异步代码块或异步函数中，咱们可使用 `await` 关键字，*等待* 某个 `future` 值（即等待其就绪）。在某个异步代码块或函数中，咱们等待某个 `future` 值的任何位置，都是该异步代码块或函数暂停及恢复的潜在位置。检查某个 `future` 值是否可用的过程，称为 *轮询，polling*。

一些别的语言，如 C# 与 JavaScript，也将 `async` 和 `await` 关键字用于异步编程。如果咱们熟悉这些语言，就可能会注意到 Rust 在行事方式，包括如何处理语法，上的一些显著差异。这是有原因的，我们将会看到！
