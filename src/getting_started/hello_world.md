...
咱们刚刚运行了一个新创建的程序，现在我们检查一下这个过程的每个步骤。

在运行 Rust 程序之前，咱们必须通过输入 `rustc` 命令，传递给他源文件名，而使用 Rust 编译器编译他，就像这样：

```console
$ rustc main.rs
```

若咱们有 C 或 C++ 背景，就会发现这与 `gcc` 或 `clang` 类似。编译成功后，Rust 会输出一个二进制可执行文件。
