...
> 注意：若出于某些原因而不愿使用 `rustup`，那么请参考 [其他 Rust 安装方式页面](https://forge.rust-lang.org/infra/other-installation-methods.html) 了解更多选项。

以下步骤将安装最新稳定版本的 Rust 编译器。Rust 的稳定性，保证了本书中所有编译成功的示例，都能继续在更新 Rust 版本中编译。不同版本的输出可能略有不同，因为 Rust 经常会改进错误信息和告警。换句话说，使用这些步骤安装的任何较新稳定 Rust 版本，都应能与本书内容所预期的一样运行。


> **命令行表示法**
>
> 在本章和全书中，我们将展示一些终端下用到的命令。咱们要敲入到某个终端中的那些行，都以 `$` 开头。咱们无需键入那个 `$`；他属于命令行提示符，用于表示每条命令的开始。不以 `$` 开头的行，通常会显示前一命令的输出。此外，特定于 `PowerShell` 的示例，会使用 `>` 而不是 `$`。
...
本书对咱们编写 Rust 代码所使用的工具不作任何假设。几乎任何文本编辑器都能胜任这项工作！不过，许多文本编辑器和集成开发环境（IDE），都内置了对 Rust 的支持。咱们可以在 Rust 官网的 [工具页面](https://www.rust-lang.org/tools) 上，找到一份相当及时的编辑器和 IDE 清单。

## 离线使用本书

在数个示例中，我们将使用标准库之外的 Rust 包。要完成这些示例，咱们需要保持网络连接，或提前下载相关依赖项。若需提前下载依赖项，可执行以下命令。（关于 `cargo` 工具及这些命令的具体作用，我们将在后续章节详细说明。）
...
这将缓存这些包的下载，因此咱们无需在后续操作中再次下载。执行此命令后，咱们无需保留 `get-dependencies` 这个文件夹。若已执行此命令，那么后续在书中使用所有 `cargo` 命令时，可通过添加 `--offline` 参数调用这些缓存版本，从而避免尝试通过网络获取资源。
