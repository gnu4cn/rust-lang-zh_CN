...
这个程序可在没有任何报错下编译，并将在我们在调用 `s.clear()` 后使用 `word` 如此行事。因为 `word` 完全没有连接 `s` 的状态，所以 `word` 仍然包含值 `3`。我们本可将值 `5` 与变量 `s` 一起使用来提取出第一个单词，但这将是个 bug，因为自从我们在 `word` 中保存 `3` 以来，`s` 的内容已经发生了变化。
